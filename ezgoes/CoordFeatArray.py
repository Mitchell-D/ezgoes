...
class CoordFeatArray:
    def __init__(self, axlabels:list, flabels:list, data=None, shape=None):
        """
        Framework for labeling and interacting with an Nd array-like datum
        structured like (C1,...,CN,F1,...,FM) for N coordinate axes C1-CN and
        F1-FN labeled feature axes.

        Axis labels are ordered and refer to the dimensions of the cfa data,
        for example a (T,Y,X,F,M) array may have axes labeled as follows:
        ("time", "lat", "lon", "feat", "metric")


        axlabels describe every axis of the cfa whether it is a coordinate or
        a feature axis

        :@param axlabels: labels associated with each axis of the data array,
            for example a (T,Y,X,F,M) array with T timesteps on a X,Y grid with
            F features and M metrics per feature would have
        """
        ##
        self._axlabels = tuple(axlabels if axlabels else ())
        self._axshape = list(axshape if not axshape is None
                else tuple(None for i in range(len(self._axlabels))))
        assert len(self._axshape)==len(self._axlabels)

        self._flabels = tuple(flabels)
        self._fshape = cshape if not cshape is None \
                else tuple(None for i in range(len(self._clabels)))
        self._data = None

        if not self._data is None:
            for caxis_size in self._cshape:
                if
            assert self._data.shape[:len(self._clabels)] == self._cshape
